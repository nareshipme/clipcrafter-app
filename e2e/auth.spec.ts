...
// Authenticated tests require a real Clerk test user.
// Set E2E_TEST_USER_EMAIL to a real user (not placeholder) to enable.
...
base.describe("Feature: Authentication – unauthenticated", () => {
  ...
  base.describe("Scenario: Sign-in page renders without errors", () => {
    base(
      "Given any visitor, When they visit /sign-in, Then the page loads without errors",
      async ({ page }) => {
...
authTest.describe("Feature: Authentication – authenticated", () => {
  authTest.describe("Scenario: Authenticated user can access /dashboard", () => {
    authTest(
      "Given authenticated user, When they visit /dashboard, Then they are not redirected to sign-in",
      async ({ page }) => {
